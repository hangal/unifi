...
public class MethodUnits implements Serializable
{
    ...
    */
	...
	//Why not do: mg.getMaxLocals() ?
	//+2 because lv is 0 based index and n_param_words>=1 beacause of this param
	int max_local_words = lv_map.highest_lv () + 2;
	real_locals = max_local_words - _n_param_words;
	...
	System.out.println("Real locals: "+real_locals+" method "+mg.getClassName()+mg.toString());
    ...
    {
	System.out.println("??????????? idx = " + idx + " no.of words : " + _n_param_words+" local vars length: "+_local_vars.length+" real local vars: "+real_locals);
